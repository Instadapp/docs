        ...
        <div class="py-16 px-4 md:px-0 md:py-0 md:w-8/12">
          <div class="mb-10 md:mb-0">
            <h2 v-if="!$fetchState.pending" class="text-blue capitalize md:leading-9 font-semibold md:mb-4">
              {{ connector.name }}</h2>
            <h2 v-else
                class="text-blue bg-gray-300 animate-pulse w-40 h-4 capitalize md:leading-9 font-semibold md:mb-4"></h2>
          ...
          <div class="font-normal mb-14 md:mb-18">
            Auth connector triggers the methods adding and removing the DSA Authorities. You can view details like
            source
            code, ABIs on Etherscan.
          </div>
          <div v-for="item in connector.functions" :id="item.name" class="mb-14 md:mb-16">
            <div class="mb-8">
          ...
          <div class="sticky top-0">
            ...
            <ul>
              <li v-for="item in connector.functions" :class="{'active': activeLink===item.name}"
                ...
                <a @click="scrollToView($event,item.name)" class="capitalize" :href="item.name">{{ item.name }}</a>
              </li>
